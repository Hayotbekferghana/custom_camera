...
void main() {
  ...
  group('Function Tests', () {
    ...
    test('Should set the zoom level', () async {
      // Arrange
      // Act
      await camera.setZoomLevel(cameraId, 2);

      // Assert
      verify(mockCameraApi.setZoomLevel(2)).called(1);
    });

    test('Should throw CameraException when illegal zoom level is supplied',
        () async {
      // Arrange
      when(mockCameraApi.setZoomLevel(-1)).thenThrow(
          PlatformException(code: 'ZOOM_ERROR', message: 'Illegal zoom error'));

      // Act & assert
      expect(
          () => camera.setZoomLevel(cameraId, -1),
