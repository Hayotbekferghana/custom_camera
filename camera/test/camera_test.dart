...
void main() {
  ...
  group('$CameraController', () {
    ...
    test(
      ...
      await cameraController.initialize();
      when(CameraPlatform.instance.setZoomLevel(mockInitializeCamera, 42))
          .thenThrow(CameraException(
        'TEST_ERROR',
        'This is a test error messge',
      ));
    ...
    test(
      ...
      await cameraController.initialize();
      await cameraController.setZoomLevel(42);

      verify(CameraPlatform.instance.setZoomLevel(mockInitializeCamera, 42))
          .called(1);
    });
    ...
    test('setExposureOffset() calls $CameraPlatform', () async {
      ...
      when(CameraPlatform.instance
          ...
          .thenAnswer((_) async => 1.0);
      when(CameraPlatform.instance
              .setExposureOffset(cameraController.cameraId, 1))
          .thenAnswer((_) async => 1.0);

      await cameraController.setExposureOffset(1);
    ...
    test('setExposureOffset() throws $CameraException on $PlatformException',
      ...
      expect(
          cameraController.setExposureOffset(1),
          throwsA(isA<CameraException>().having(
            (CameraException error) => error.description,
            'TEST_ERROR',
            'This is a test error message',
    ...
    test(
      ...
      when(CameraPlatform.instance
          ...
          .thenAnswer((_) async => 1.0);
      when(CameraPlatform.instance
              .setExposureOffset(cameraController.cameraId, 0))
          .thenAnswer((_) async => 0.0);
      when(CameraPlatform.instance
              .setExposureOffset(cameraController.cameraId, -1))
          .thenAnswer((_) async => 0.0);
      when(CameraPlatform.instance
              .setExposureOffset(cameraController.cameraId, 2))
          .thenAnswer((_) async => 0.0);

      expect(
          cameraController.setExposureOffset(3),
          throwsA(isA<CameraException>().having(
            (CameraException error) => error.description,
      ...
      expect(
          cameraController.setExposureOffset(-2),
          throwsA(isA<CameraException>().having(
            (CameraException error) => error.description,
            'exposureOffsetOutOfBounds',
            'The provided exposure offset was outside the supported range for this device.',
      ...
      await cameraController.setExposureOffset(0);
      await cameraController.setExposureOffset(-1);
      await cameraController.setExposureOffset(2);
